...
export const SlideEditor: React.FC<SlideEditorProps> = ({
  ...
  const saveMediaLibrary = (library: MediaLibraryItem[]) => {
    try {
      // Filtrar Object URLs antes de salvar no localStorage (eles não podem ser serializados)
      const libraryToSave = library.filter(item => !item.url.startsWith('blob:'));
  ...
  const addToMediaLibrary = async (file: File): Promise<string> => {
    try {
      ...
      if (file.type.startsWith('image/')) {
        ...
        // Usar Object URL em vez de base64 para vídeos
  ...
  const removeFromMediaLibrary = (id: string) => {
    // Revogar Object URL se necessário para liberar memória

      ...
      if (element.type === 'video') {
        ...
        if (existingVideo && existingVideo.src === element.url) {
          ...
          video.addEventListener("canplay", () => {
            // Iniciar vídeos se autoPlay estiver ativado ou se isVideoPlaying for true (para reprodução manual)
      ...
      if (isVideoPlaying) {
        for (const [id, video] of videoRefs.current) {
          try {
            ...
            if (element?.autoPlay || isVideoPlaying) { // Permitir reprodução manual se isVideoPlaying for true
              video.currentTime = 0;
              await video.play();
                  ...
                  <video 
                    ...
                    ref={(el) => {
                      if (el) {
                        ...
                        // Reproduzir se autoPlay estiver ativado ou se isVideoPlaying for true
                        if ((element.autoPlay && isVideoPlaying) || (!element.autoPlay && isVideoPlaying)) {
                    ...
                    src={element.url}
                    className="max-w-full max-h-full object-contain"
                    autoPlay={false} // Sempre false para controle manual
                    ...
                    style={{ imageRendering: 'high-quality' }}
                    onClick={() => {
                      // Permitir clique no vídeo para iniciar/pausar

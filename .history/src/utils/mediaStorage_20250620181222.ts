...
class MediaStorageManager {
  private readonly MAX_STORAGE_SIZE = 900 * 1024 * 1024; // 900MB (deixa margem)
  private readonly MAX_VIDEO_SIZE = 50 * 1024 * 1024; // 50MB por vídeo
  ...
  async addMedia(file: File): Promise<MediaFile> {
    try {
      ...
      const existing = library.find(item => 
        item.name === file.name && 
        Math.abs(item.size - mediaFile.size) < 10000
      );
